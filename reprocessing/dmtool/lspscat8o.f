      SUBROUTINE LSPSCAT( sig, sigsig, n1, n2, n3, n4, nm11, nm12, nm13, 
     & nm14, nm21, nm22, nm23, nm24, nm31, nm32, nm33, nm34, nm41, nm42, 
     & nm43, nm44, lhiggs, shiggs, halpha, sbeta, mtin, mbin, ul, ur,
     & dl, dr, b1, b2, t1, t2, tt, tb )

      REAL*8 n1, n2, n3, n4
      REAL*8 sig, sigsig, nm11, nm12, nm13, nm14, nm21,
     & nm22, nm23, nm24, nm31, nm32, nm33, nm34, nm41, nm42, nm43, nm44,
     & lhiggs, shiggs, halpha, sbeta, mtin, mbin, ul, ur, dl, dr,
     & b1, b2, t1, t2, tt, tb
      REAL*8 s2out, ss2out, s3out, ss3out
c
...
c
     ...
     &      At, Ab, Ag, M2x, absmx, sb, Ma, Mo, Mt, Mb,
     &      thmixt, thmixb,ratio,sigmacdms,sigbr2,
     $      msur1,msul1,msdr1,msdl1
      REAL*8  andy1, andy2,y,zf,bdbu,mumd,msmd,r,
     $        mubu,mdbd,msbs,sigy,ftup0,ftdp0,ftsp0,
     $        sig0,sigsig0,sigmumd,sigmsmd,sigftup,
...
c
      COMMON /fcom/ s2out, ss2out, s3out, ss3out
...
c      PRINT*, evals(1),evals(2),evals(3),evals(4)
c      PRINT*, evecs(1,1),evecs(1,2),evecs(1,3),evecs(1,4)
c      PRINT*, evecs(2,1),evecs(2,2),evecs(2,3),evecs(2,4)
c      PRINT*, evecs(3,1),evecs(3,2),evecs(3,3),evecs(3,4)
c      PRINT*, evecs(4,1),evecs(4,2),evecs(4,3),evecs(4,4)
c      PRINT*, sinb,mtop,mbot
c      PRINT*, msul, msdl, msur, msdr 
c      PRINT*, msbr, msbl, mstr, mstl, thmixt, thmixb
...
c      write(6,*)'sigmapiN and its error:'
c      read(5,*)sig,sigsig
...
c
     ...
     &    (1/mh1**2 - 1/mh2**2) - z321*( (DCOS(alpha))**2/mh1**2 +
     &    (DSIN(alpha))**2/mh2**2 ))
c        PRINT*,'alph2u = ',alph2u
c        PRINT*,'alph2d = ',alph2d
c        PRINT*,'alph3u = ',alph3u
c
c
...
ccccccccccccccccccccccccccccccc
        s2out = sigma2*0.38937932E9 
        ss2out = sigsigma2*0.38937932E9 
        s3out = sigma3*0.38937932E9
        ss3out = sigsigma3*0.38937932E9    

c        WRITE(10,*) s2out, ss2out, 
c     $        s3out, ss3out    
c        Print*,'sig,y,r,bdbu,ftp,ftd,fts',sig,y,r,
c     $          bdbu,ftup,ftdp,ftsp
c        Print*,'spin dependent cross section (in pb) =',
c     &     s2out, '+/-', ss2out
c        Print*,'scalar cross section (in pb) =',
c     &     s3out, '+/-', ss3out
